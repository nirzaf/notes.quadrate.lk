...
QNotes records Vault events in `notesdb.vault_audit_events`. The event row is committed in the same database transaction as a successful secret mutation or reveal, so a local audit failure rolls back secret delivery. Events contain actor and resource identifiers, action, result, purpose, request ID, operation ID, policy revision, and timestamps; secret values and request payloads are never stored.
...
Every event is copied by an insert trigger into `notesdb.vault_audit_outbox`. The outbox payload contains only the audit fields and is claimed in bounded batches. An external operator-owned exporter must deliver each payload to independently controlled append-only storage, acknowledge only after durable delivery, and call the retry RPC with bounded error text when delivery fails. Local audit commits do not wait for that network exporter; pending rows and attempts provide the lag signal.
