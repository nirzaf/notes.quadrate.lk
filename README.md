...
The browser subscribes to the private `user:<user-id>:notes` Realtime channel and receives metadata-only `note.changed` events. IndexedDB stores drafts separately from an account-specific, bounded cache of recent authoritative note bodies and a local lexical search record. Each recovery page fetches authorized note content and commits cache records, tombstones, and its sync cursor in one transaction, so a crash replays the page safely. Access tokens and Vault values are not stored in IndexedDB or the service-worker cache.
