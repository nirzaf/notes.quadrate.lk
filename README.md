...
CI runs the core typecheck/unit/Edge/build/generated-parity checks on the repository's labeled self-hosted runner. Local Supabase SQL, focused E2E suites, and the path-gated search regression job are opt-in through the repository Actions variable `QNOTES_RUN_LOCAL_SUPABASE=true`, because they require Docker user namespaces. A successful push to `master` runs the production CD job after core passes; when the optional jobs are enabled, they must pass too. Deployment applies migrations, deploys Supabase Edge Functions, publishes Cloudflare Pages, and verifies the live health endpoints. See [.github/SELF_HOSTED_RUNNER.md](.github/SELF_HOSTED_RUNNER.md) for runner setup, the opt-in local jobs, and production secrets. The available check names are `CI / core`, `CI / integration`, and `CI / search regression`; enable the variable before making the latter two required in branch protection. See [SECURITY.md](SECURITY.md) for the full-history secret-scan and private vulnerability-reporting procedure.
