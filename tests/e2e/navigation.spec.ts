...
test('restores submitted search context after opening a result and refreshing', async ({ page }) => {
  ...
  const assertNoPageErrors = capturePageErrors(page);
  await signInPage(page);
  const search = page.getByLabel('Search notes').first();
  await search.fill(marker);
  await search.press('Enter');
  await expect(page).toHaveURL(new RegExp(`[?&]q=${marker}`));
  await expect(page.locator('.q-search-result').filter({ hasText: note.title })).toBeVisible({ timeout: 15_000 });
  await page.locator('.q-search-result').filter({ hasText: note.title }).getByRole('button', { name: 'Open matching section' }).click();
  await expect(page).toHaveURL(new RegExp(`/notes/${note.id}`));
  await page.goBack();
  await expect(page.getByLabel('Search notes').first()).toHaveValue(marker);
  await page.reload();
  await expect(page.getByLabel('Search notes').first()).toHaveValue(marker);
  assertNoPageErrors();
});
