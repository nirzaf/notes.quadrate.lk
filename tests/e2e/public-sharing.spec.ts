...
import { apiJson, createNoteApi, createPublicShareApi, getNoteApi, OWNER, publicApiJson, resolvePublicShareApi, signInPage, signInSession } from './helpers';
...
test('renders a saved note publicly through the fragment without private API requests', async ({ browser }) => {
  ...
  const getResolver = await publicApiJson(`/public/share/resolve?token=${encodeURIComponent(share.token)}`, { headers: { Accept: 'text/markdown' } });
...
test('rotates and revokes links while keeping the old secret unusable', async () => {
  ...
  const getResolver = await publicApiJson(`/public/share/resolve?token=${encodeURIComponent(second.token)}`, { headers: { Accept: 'text/markdown' } });
  expect([404, 405]).toContain(getResolver.response.status);
