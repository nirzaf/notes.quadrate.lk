...
export function HomePage(): JSX.Element {
  ...
  const notesQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => api.listNotes({ limit: 50, ...(pageParam ? { cursor: pageParam } : {}) }),
  ...
  const notes = useMemo(() => [...(notesQuery.data?.pages.flatMap((page) => page.items) ?? [])].sort((left, right) => Date.parse(right.updatedAt) - Date.parse(left.updatedAt)), [notesQuery.data?.pages]);
