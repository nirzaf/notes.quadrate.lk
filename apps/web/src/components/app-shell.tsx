...
import { ArrowLeft, FileText, LogOut, Menu, MoreHorizontal, Plus, Search, Settings2, Trash2 } from 'lucide-react';
import { api } from '../api';
...
export function AppShell({ title = 'Quadrate Notes', notes = [], sidebarNotes: sidebarNotesProp, activeNoteId, selectedNotebookId: selectedNotebookIdProp, onNotebookSelect, onNew, onSelectNote, onRealtimeEvent, onRealtimeReconnect, mobileBack = false, mobileBackLabel = 'Notes', children }: AppShellProps): JSX.Element {
  ...
  return <div className="q-app">
    ...
    <Sheet open={moreOpen} onOpenChange={setMoreOpen}><SheetContent id="mobile-more-sheet" side="bottom" className="q-mobile-more"><DialogHeader><DialogTitle>More workspace tools</DialogTitle><DialogDescription>Notebooks, integrations, Trash, and account actions.</DialogDescription></DialogHeader><NotebookList idPrefix="mobile" notebooks={notebooksQuery.data?.items ?? []} selectedNotebookId={selectedNotebookId} onSelect={(notebookId) => { selectNotebook(notebookId); setMoreOpen(false); }} onCreate={createNotebook} /><div className="q-mobile-more-links"><Link to="/settings/integrations" onClick={() => setMoreOpen(false)}><Settings2 size={17} aria-hidden="true" /><span>Integrations</span></Link><Link to="/trash" onClick={() => setMoreOpen(false)}><Trash2 size={17} aria-hidden="true" /><span>Trash</span></Link><button type="button" onClick={() => { setMoreOpen(false); void signOut().catch((error: unknown) => toast(error instanceof Error ? error.message : 'Unable to sign out.', 'error')); }}><LogOut size={17} aria-hidden="true" /><span>Sign out</span></button></div></SheetContent></Sheet>
  </div>;
}
