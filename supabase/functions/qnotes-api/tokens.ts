...
export async function listTokens(context: Context): Promise<Response> {
  ...
  requireUserJwt(auth);
  const { data, error } = await appDbClient.from('api_tokens').select('id, name, token_prefix, scopes, expires_at, last_used_at, revoked_at, created_at').eq('owner_id', auth.userId).order('created_at', { ascending: false });
...
export async function createToken(context: Context): Promise<Response> {
  ...
  const { data, error } = await appDbClient.from('api_tokens').insert({ owner_id: auth.userId, name: input.name, token_prefix: tokenPrefix(token), token_hash: hash, scopes: input.scopes, expires_at: input.expiresAt }).select('id, name, token_prefix, scopes, expires_at, last_used_at, revoked_at, created_at').single();
