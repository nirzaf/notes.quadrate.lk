...
export async function authenticateRequest(request: Request): Promise<AuthContext> {
  ...
  if (isPersonalToken(credential)) {
    ...
    const { data, error } = await serviceClient.from('api_tokens').select('id, owner_id, scopes, expires_at, revoked_at').eq('token_hash', tokenHash).maybeSingle();
    ...
    await serviceClient.from('api_tokens').update({ last_used_at: new Date().toISOString() }).eq('id', data.id);
