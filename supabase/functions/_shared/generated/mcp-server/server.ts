...
export function createQNotesMcpServer(client: QNotesClient & ReadQNotesClient, profile: McpProfile = 'read'): McpServer {
  ...
  if (profile === 'write') {
    ...
    }, (args: Record<string, unknown>) => captureNoteTool(writeClient, args as Parameters<typeof captureNoteTool>[1]));
    server.registerTool('append_note', {
      ...
      annotations: { readOnlyHint: false, openWorldHint: false },
    }, (args: Record<string, unknown>) => appendNoteTool(writeClient, args as Parameters<typeof appendNoteTool>[1]));
    server.registerTool('update_note', {
      ...
      annotations: { readOnlyHint: false, openWorldHint: false },
    }, (args: Record<string, unknown>) => updateNoteTool(writeClient, args as Parameters<typeof updateNoteTool>[1]));
  }
  return server;
}
