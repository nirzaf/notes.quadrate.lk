...
create unique index notes_active_owner_slug_key
  on public.notes (owner_id, lower(slug))
  where deleted_at is null;
create index notes_owner_updated_key on public.notes (owner_id, updated_at desc, id desc);
create index notes_owner_deleted_key on public.notes (owner_id, deleted_at);
create index notes_tags_gin_key on public.notes using gin (tags);
...
create index note_blocks_owner_note_position_key on public.note_blocks (owner_id, note_id, position);
...
create index search_documents_vector_gin_key on public.search_documents using gin (search_vector);
create index search_documents_embedding_hnsw_key on public.search_documents using hnsw (embedding extensions.vector_ip_ops);
create index search_documents_owner_note_key on public.search_documents (owner_id, note_id);
create index search_documents_owner_embedding_status_key on public.search_documents (owner_id, embedding_status);
...
create index attachments_owner_note_created_key on public.attachments (owner_id, note_id, created_at);
create index attachments_owner_status_key on public.attachments (owner_id, extraction_status);
...
create index note_mutations_created_key on public.note_mutations (created_at);

create trigger notes_set_updated_at before update on public.notes
for each row execute function public.qnotes_set_updated_at();
create trigger note_blocks_set_updated_at before update on public.note_blocks
for each row execute function public.qnotes_set_updated_at();
