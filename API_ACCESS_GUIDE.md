...
To connect it in Gemini Spark, open Connected Apps, enter the endpoint under **Custom apps for Spark**, expand **Advanced Settings**, and choose **Next**. This endpoint uses Gemini’s manual OAuth-client path: enter `qnotes-gemini` as **Client ID** and paste the personal `qnt_...` token as **Client secret**. The server validates the OAuth authorization-code and PKCE exchange, then gives Gemini the same read-only bearer credential; no write profile is available. The token is shown in full only once, so revoke it from [Personal API tokens](https://notes.quadrate.lk/settings/tokens) if it is exposed or no longer needed. The hosted endpoint accepts Google’s OAuth redirect hosts only and does not support dynamic client registration.
