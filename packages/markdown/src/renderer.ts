...
interface MarkdownEngine {
  ...
  renderer: { rules: Record<string, (tokens: MarkdownToken[], index: number) => string> };
}

const MarkdownIt = MarkdownItImport as unknown as new (options: { html: boolean; linkify: boolean; typographer: boolean }) => MarkdownEngine;
