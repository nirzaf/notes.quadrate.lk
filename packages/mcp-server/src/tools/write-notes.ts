...
export async function captureNoteTool(client: WriteQNotesClient, args: { title: string; contentMarkdown: string; tags?: string[]; slug?: string; notebookId?: string | null; dedupeKey?: string; deviceId?: string; mutationId?: string }, options?: WriteToolOptions) {
  const input: CreateNoteInput = { title: args.title, contentMarkdown: args.contentMarkdown, tags: args.tags ?? [], ...(args.notebookId !== undefined ? { notebookId: args.notebookId } : {}), ...(args.dedupeKey !== undefined ? { dedupeKey: args.dedupeKey } : {}), deviceId: deviceId(args, options), mutationId: args.mutationId ?? randomUUID() };
