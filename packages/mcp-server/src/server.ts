...
import { DEFAULT_MCP_CONTENT_MAX_BYTES, MAX_BLOCK_KEY_LENGTH, MAX_DEDUPE_KEY_LENGTH, MAX_MARKDOWN_CODE_UNITS, MAX_OUTLINE_SECTION_ID_LENGTH, MAX_PATCH_REPLACEMENT_BYTES, MAX_SEARCH_CURSOR_LENGTH, MAX_SEARCH_LIMIT, MAX_SEARCH_QUERY_LENGTH, MAX_SLUG_LENGTH, MAX_SYNC_LIMIT, MAX_TAG_COUNT, MAX_TAG_LENGTH, MAX_TITLE_LENGTH, MAX_VAULT_BATCH_REVEAL, MAX_VAULT_DESCRIPTION_LENGTH, MAX_VAULT_ENVIRONMENT_NAME_LENGTH, MAX_VAULT_PROJECT_NAME_LENGTH, MAX_VAULT_PURPOSE_LENGTH, MAX_VAULT_SECRET_BYTES, MAX_VAULT_SECRET_NAME_LENGTH } from '@qnotes/shared';
import { MCP_CONTRACT_VERSION, capabilitiesSchema, captureAcknowledgmentSchema, mutationAcknowledgmentSchema, mutationStatusSchema, noteBlockSchema, noteOutlineSchema, noteSectionPatchPreviewSchema, notebookListSchema, publicShareSchema, publicSharedNoteSchema, searchContextSchema, searchResponseSchema, strictInput, syncPageSchema, vaultEnvironmentsSchema, vaultMetadataSchema, vaultMutationStatusSchema, vaultProjectsSchema, vaultSecretBatchSchema, vaultSecretSchema, vaultSecretsSchema } from './contracts.ts';
...
import { getCapabilitiesTool, getMutationStatusTool, getNoteOutlineTool, listNoteChangesTool, patchNoteSectionTool, previewNoteSectionTool } from './tools/editing.ts';
...
export const READ_TOOL_NAMES = ['get_capabilities', 'search_notes', 'read_note_context', 'get_block', 'list_notebooks', 'list_note_changes', 'get_note_outline', 'get_mutation_status', 'resolve_public_share'] as const;
export const SHARE_TOOL_NAMES = ['create_public_share'] as const;
export const WRITE_TOOL_NAMES = ['capture_note', 'append_note', 'update_note', 'preview_note_section', 'patch_note_section', 'delete_note', 'restore_note', 'move_note_to_notebook'] as const;
...
const outlineInputSchema = strictInput({ noteRef: z.string().min(1).max(200) });
const changesInputSchema = strictInput({ cursor: z.string().min(1).max(MAX_SEARCH_CURSOR_LENGTH).optional(), limit: z.number().int().min(1).max(MAX_SYNC_LIMIT).optional() });
const mutationStatusInputSchema = strictInput({ mutationId: z.string().uuid() });
const patchNoteSectionInputSchema = strictInput({ noteId: z.string().uuid(), sectionId: z.string().min(1).max(MAX_OUTLINE_SECTION_ID_LENGTH), expectedVersion: z.number().int().min(1), expectedContentHash: z.string().regex(/^[a-f0-9]{64}$/i), replacementMarkdown: utf8AtMost(MAX_PATCH_REPLACEMENT_BYTES).max(MAX_MARKDOWN_CODE_UNITS), mutationId: z.string().uuid() });
...
export function createQNotesMcpServer(client: QNotesClient & ReadQNotesClient, profile: McpProfile = 'read', options: QNotesMcpServerOptions = {}): McpServer {
  ...
  const supportedOperations = profile === 'write' && options.allowPublicShare === true ? [...PROFILE_TOOL_NAMES.write, 'create_public_share'] : PROFILE_TOOL_NAMES[profile];
  server.registerTool('get_capabilities', { description: 'Discover the effective QNotes profile, scopes, bounded operations, and response limits.', inputSchema: noInputSchema, outputSchema: capabilitiesSchema, annotations: readAnnotations }, safeTool(() => getCapabilitiesTool(client, profile, supportedOperations)));
  ...
  server.registerTool('list_note_changes', { description: 'Enumerate a bounded, authorized page of note changes using an opaque cursor.', inputSchema: changesInputSchema, outputSchema: syncPageSchema, annotations: readAnnotations }, safeTool((args: Record<string, unknown>) => listNoteChangesTool(client, args as Parameters<typeof listNoteChangesTool>[1])));
  server.registerTool('get_note_outline', { description: 'Return bounded section ranges and block identifiers for one exact note without returning section bodies.', inputSchema: outlineInputSchema, outputSchema: noteOutlineSchema, annotations: readAnnotations }, safeTool((args: Record<string, unknown>) => getNoteOutlineTool(client, args as Parameters<typeof getNoteOutlineTool>[1])));
  server.registerTool('get_mutation_status', { description: 'Recover the durable status of one caller-owned note mutation by its original mutation ID.', inputSchema: mutationStatusInputSchema, outputSchema: mutationStatusSchema, annotations: readAnnotations }, safeTool((args: Record<string, unknown>) => getMutationStatusTool(client, args as Parameters<typeof getMutationStatusTool>[1])));
  ...
  if (profile === 'write') {
    ...
    server.registerTool('preview_note_section', { description: 'Preview one exact Markdown section replacement against an expected version and hash without writing the note.', inputSchema: patchNoteSectionInputSchema, outputSchema: noteSectionPatchPreviewSchema, annotations: readAnnotations }, safeTool((args: Record<string, unknown>) => previewNoteSectionTool(writeClient, args as Parameters<typeof previewNoteSectionTool>[1], options)));
    server.registerTool('patch_note_section', { description: 'Replace one exact Markdown section body after matching its outline section ID, version, and content hash.', inputSchema: patchNoteSectionInputSchema, outputSchema: mutationAcknowledgmentSchema, annotations: writeAnnotations }, safeTool((args: Record<string, unknown>) => patchNoteSectionTool(writeClient, args as Parameters<typeof patchNoteSectionTool>[1], options)));
