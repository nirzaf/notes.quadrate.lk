...
test('accepts the least-privilege share scope and generates a share MCP profile', () => {
  ...
  assert.throws(() => validateTokenInput({ name: 'Invalid access', scopes: ['notes:read'], access: null }), /access\.mode must be account or notebooks/);
  assert.throws(() => validateTokenInput({ name: 'Invalid access', scopes: ['notes:read'], access: { mode: 'notebooks', notebookIds: [], allowUnfiled: null } }), /access\.allowUnfiled must be a boolean/);
